...
int32_t main() {
    ...
    {
        int r = nums[i];
        int remainder = 1 + 4 * r + 4 * int(r / sqrtl(2));
        totrem += remainder;

        if (remainders[remainder % 8].size() > 8) continue;
        remainders[remainder % 8].push_back(getfaster(r));
    }

    vp2 rems;
    rep(i, remainders.size())
    {
        repe(v, remainders[i])
        {
            assert(v % 8 == i);
            rems.emplace_back(i, v);
        }
    }

    int ans = inf;

    rep(mask, 1 << rems.size())
    {
        int t = 0;
        int r = 0;
        rep(i, rems.size())
